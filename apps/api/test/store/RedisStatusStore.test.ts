...
describe('RedisStatusStore', () => {
  ...
  it('salva e lê o snapshot atual', async () => {
    ...
    await store.saveSnapshot('producao', [makeService(), makeService({ id: 'NFe:MG', uf: 'MG' })]);

    const snapshot = await store.getSnapshot('producao');
    expect(snapshot).toHaveLength(2);
    expect(await store.getService('producao', 'NFe:SP')).toMatchObject({ uf: 'SP' });
  });
  ...
  it('acumula histórico e poda pontos fora da janela de retenção', async () => {
    ...
    now = 0;
    await store.saveSnapshot('producao', [makeService({ state: 'DOWN' })]);
    now = 500;
    await store.saveSnapshot('producao', [makeService({ state: 'OPERATIONAL' })]);
    now = 2000; // ponto t=0 já saiu da janela (cutoff = 1000)
    await store.saveSnapshot('producao', [makeService({ state: 'SLOWDOWN' })]);

    const history = await store.getHistory('producao', 'NFe:SP', '1h');
  ...
  it('não falha ao salvar snapshot vazio', async () => {
    ...
    await expect(store.saveSnapshot('producao', [])).resolves.toBeUndefined();
  });
});
